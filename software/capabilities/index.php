<!DOCTYPE html>
<html lang="en-US">
<head>
    <?php
    include "../../assets_header.php"
    ?>
    <link rel="stylesheet" type="text/css" href="<?=baseUrl()?>/themes/the-interwebs/dist/css/pages/structured-page56bd.css" />
    <link rel="stylesheet" type="text/css" href="<?=baseUrl()?>/themes/the-interwebs/dist/css/elements/template-image-flushddc3.css" />
    <style>
        ul {
            padding-left: 20px !important;
        }
        .container .vam {
            width: 100% !important;
        }
        .image-flush-right-text-left h2 {
            display: inline-block !important;
            vertical-align: top !important;
            margin-right: 25px !important;
            font-size: 25px !important;
            width: 12% !important;
        }
        .container ul {
            display: inline-block !important;
            width: 70% !important;
        }
    </style>
    <?php if(isset($_GET["id"])){ ?>
        <script src="../../assets/jquery.min.js"></script>
        <script type="text/javascript">
            $(document).ready(function (){
                $('html, body').animate({
                    scrollTop: $("#<?=$_GET["id"]?>").offset().top
                }, 1000);
            })

        </script>
    <?php } ?>
</head>
<body class="staircasepage StaircasePage Action-index lead-data-unknown locale-us" dir="ltr">
<?php
include "../../header.php"
?>
      <article class="header-wrapper" style="margin-top: 65px">
            <section class="image-cover background gradient marketo"


                     style="background-color: #8F191F;"

                >
                <div id="rtp-10800-header" class="rtp-container container hero Medium  tac">




                                <h1 class="adobe-header">Capabilities</h1>

                            <h2 >Transform your self-directed services with our ever-growing list of capabilities

                            </h2>


                </div>
            </section>
        </article>
    

    

<main id="layout-wrapper">

	<div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

<section class="image-flush background white rtp-container" id="Onboarding"
		 style="">
    <div class="container image-flush-right-text-left">
		<div class="content vam">
			<h2>Onboarding</h2>
            <ul>
                <li>Digital online self-service for Customers</li>
                <li>Dynamic onboarding journeys</li>
                <li>Digital documents and digital signing</li>
                <li>Integration to external systems such as CRM and document management</li>
                <li>Integrated help including automatically triggered guidance videos
                </li>
            </ul>
		</div>

    </div>
</section>
</div>

    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Budgeting"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Budgeting</h2>
                    <ul>
                        <li>Advanced dynamic budgeting capability</li>
                        <li>Budget for Staff, Expenses and for support from Service Providers</li>
                        <li>Graphical analysis of budgeted spending</li>
                        <li>Proportional spending trend analysis and tracking</li>
                        <li>Ability to reset the budget and start from a new date</li>
                        <li>Easy to use tools to optimise the spending of funding on staffing
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Funding"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Funding</h2>
                    <ul>
                        <li>Funding can be entered directly into the system by staff, uploaded from a spreadsheet or entered via integration with an external system</li>
                        <li>Funding start dates and end dates can be configured</li>
                        <li>Automated reminder notices when funding renewal is due</li>
                        <li>Customers can see funding details</li>
                        <li>Zero balance funding supported</li>
                        <li>Customers can have multiple fundings
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Spending"
                 style="">
            <div class="container image-flush-right-text-left">

                <div class="content vam">
                    <h2>Spending Management</h2>
                    <ul>
                        <li>Automated analysis of spending on a daily basis</li>
                        <li>Customer spending analysis self-service</li>
                        <li>Easy to follow Customer statement</li>
                        <li>Automated issuance of Customer statements to those Customers who want to regularly receive their statement</li>
                        <li>Accessible version of the Customer Statement</li>
                        <li>Detailed breakdown of Customer spending at a transaction level</li>
                        <li>Spending analysis graphs</li>
                        <li>Ability to extract transactions for analysis</li>
                        <li>Ability to manually upload transactions from an external system</li>
                        <li>Ability to upload transactions from an external system via an interface</li>
                        <li>Predefined notifications and communications processes triggered by irregular spending</li>
                        <li>Both Customers and Staff are made aware of spending issues</li>
                        <li>Claims payment limitations can be applied where spending issues occur</li>
                        <li>Claims that are ‘in process’ are included in spending management to ensure accuracy of the remaining balance
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Payments"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Payments Module</h2>
                    <ul>
                        <li>Customer can enter claims for expenses themselves</li>
                        <li>Ability to pay / reimburse the Customer</li>
                        <li>Ability to pay a service provider directly on invoice</li>
                        <li>Ability to setup Service Providers to be selectable from a list of providers</li>
                        <li>Ability to review and rate Service Providers</li>
                        <li>Ability for Staff to review and process Claims</li>
                        <li>Ability for Staff to query sand reject a claim within the system</li>
                        <li>Ability to automatically process some claims
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Workforce"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Workforce Scheduling & Rostering</h2>
                    <ul>
                        <li>Customers can create their own staff schedule and roster</li>
                        <li>Ability to schedule activities and work shifts</li>
                        <li>Ability to add recurring activities and work shifts</li>
                        <li>Ability to invite staff to work shifts</li>
                        <li>Ability to cost a roster</li>
                        <li>Ability to notify Staff of shift assignment via different methods</li>
                        <li>Ability to set reminders</li>
                        <li>Ability to attach documents to an activity or work shift</li>
                        <li>Ability to convert the worked roster to Claims / Timesheets
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Compliance"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Audit & Compliance</h2>
                    <ul>
                        <li>Detailed audit trail logging of changes to key data such as timesheets and claims</li>
                        <li>Ability to extract the spending history for an account into an audit report</li>
                        <li>Ability for an auditor to be able to access a specific Customer account in ‘read-only’ to support spending investigations
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Application"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Web Application</h2>
                    <ul>
                        <li>Bespoke self service application available via a web application</li>
                        <li>Support for all main internet browsers</li>
                        <li>Microsoft technology supported by the Microsoft Azure Cloud  security
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Integration"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Integration</h2>
                    <ul>
                        <li>Support for REST API integration</li>
                        <li>Support for SFTP File Integration</li>
                        <li>Support for Excel and .CSV File integration</li>
                        <li>Active monitoring of integrations
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Reporting"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Reporting</h2>
                    <ul>
                        <li>In-built suite of reports enabling the extraction of key data (e.g. Audit Report)</li>
                        <li>Ability to extract Customer transaction data from the system for analysis</li>
                        <li>Support for Microsoft Power BI
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Monitoring"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Monitoring</h2>
                    <ul>
                        <li>In-built monitoring service</li>
                        <li>Ability to check the status of critical system processes and services in a dedicated dashboard</li>
                        <li>Active monitoring of connected systems and interfaces including automated alerts when interfaces become unstable or stop working</li>
                        <li>Support for Microsoft Azure Cloud monitoring services</li>
                        <li>Support for integration to service management systems (e.g. JIRA)
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>





    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Accessibility"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Accessibility</h2>
                    <ul>
                        <li>UI support for Dyslexia</li>
                        <li>UI support for Cursor size changes</li>
                        <li>UI support for colour contrast inversion</li>
                        <li>UI support for dynamic Zoom</li>
                        <li>Tested to work with all main screenreaders (JAWS, NVDA, Microsoft Narrator, etc)
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Security"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Security</h2>
                    <ul>
                        <li>Identify Server 4 authentication</li>
                        <li>Role based access model</li>
                        <li>Microsoft Azure Cloud – Azure Firewall</li>
                        <li>Microsoft Azure Cloud – Azure DDoS Protection</li>
                        <li>Regularly tested and verified by an independent 3rd Party testing organisation
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Authentication"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Authentication</h2>
                    <ul>
                        <li>Identify Server 4 authentication</li>
                        <li>Support for single-sign on across self-service applications</li>
                        <li>Support for multi-factor authentication</li>
                        <li>Support for external authentication integration (LDAP, Microsoft Active Directory)</li>
                        <li>Password complexity checker</li>
                        <li>Automated support for regular password renewal (Default 180 days)
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Smartphone"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Smartphone Application</h2>
                    <ul>
                        <li>Native Google Android Smartphone Application</li>
                        <li>Native Apple IOS Smartphone Application</li>
                        <li>Support for push notifications</li>
                        <li>Ability to disable functions, modules or applications as required</li>
                        <li>Ability to force application update</li>
                        <li>Mobile App emulation console for Customer Support staff</li>
                        <li>Single Administration application across both Android and IOS Smartphone Applications
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Automation"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Automation</h2>
                    <ul>
                        <li>Automated processing of Timesheets</li>
                        <li>Automated processing of Expenses Claims</li>
                        <li>Automated issuance of Customer spending Statements</li>
                        <li>Automated issuance of Employee Payslips</li>
                        <li>Automated analysis of spending behaviour</li>
                        <li>Automated reminders and communications processes to support payroll processing
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Module"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Spending Compliance Module</h2>
                    <ul>
                        <li>Funder specific view of spending compliance</li>
                        <li>Aggregation of bank transactions leveraging bank account feeds / integration</li>
                        <li>Summary dashboard for Staff of Customers with details of spending</li>
                        <li>Analysis of spending transactions using predefined rules
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Purchasing"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Purchasing Decision Making</h2>
                    <ul>
                        <li>Customers can access an online purchasing support tool to provide details in support of a purchased item</li>
                        <li>Customer can enter a proposed purchase item and submit it to clarify if it can be purchased</li>
                        <li>Notes can be added to an Account to detail purchasing exceptions
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Payroll"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Payroll</h2>
                    <ul>
                        <li>Time and attendance self-service for Employers</li>
                        <li>Time and attendance self-service for Employees</li>
                        <li>Payroll email inbox can be consumed into a processing queue</li>
                        <li>Customers can enter details for a new Employee including loading key documents</li>
                        <li>Customers can terminate Employees</li>
                        <li>Advanced claims queueing screens for streamlined processing</li>
                        <li>Ability to record account and claim level notes</li>
                        <li>Advanced audit log to track changes to Claims and Employees</li>
                        <li>Pay rate history support</li>
                        <li>Payslips self-service for Employers</li>
                        <li>Payslips self-service for Employees</li>
                        <li>Payslips issuance</li>
                        <li>Leave Balances self-service for Employers</li>
                        <li>Automated review and approval of timesheets including automated insertion into a payroll system</li>
                        <li>Advanced integration to an external payroll system</li>
                        <li>Automated monitoring of external payroll system interfaces
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Direct"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Direct Payments</h2>
                    <ul>
                        <li>Dynamic tool for creating a direct payments disbursement plan based on budgeted items</li>
                        <li>Ability to map budgeted spending to a purpose or outcome</li>
                        <li>Ability to manage actual spending into the direct payments plan to avoid overspending</li>
                        <li>Summary dashboard of direct payments participants for staff that indicates spending status, bank account balance, etc</li>
                        <li>Ability to re-process direct payments disbursements</li>
                        <li>Ability to process one-off direct payments disbursements</li>
                        <li>Automated creation of direct payments disbursements into a financial system via integration</li>
                        <li>Automated creation of direct payments disbursements in a banking system via integration
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Funding"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Direct Payments Funding Bank Account Management</h2>
                    <ul>
                        <li>Bank account integration to aggregate bank transactions</li>
                        <li>Automated analysis of bank transactions to identify irregular spending, transfers and withdrawals using ‘rules’ based evaluation</li>
                        <li>Ability to apply global transaction analysis rules or to customise rules for a specific Customer</li>
                        <li>Automated analysis of bank balance to identify excess balances</li>
                        <li>Automated prompting of Customer to qualify a bank transaction</li>
                        <li>Ability for the Customer to add a detailed note to a bank transaction</li>
                        <li>Ability for the Customer to attach an invoice to a bank transaction</li>
                        <li>Ability for the Customer to map / classify a bank transaction to a purpose of goal</li>
                        <li>Ability for the Customer to opt to automatically classify bank transactions once they have been classified for the first time
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
    <div class='js-scroll-to-section' id='sectionID-4450' data-scroll-id='sectionID-4450' data-title='Increase Sales Productivity'>

        <section class="image-flush background white rtp-container" id="Personal"
                 style="">
            <div class="container image-flush-right-text-left">
                <div class="content vam">
                    <h2>Personal Assistant Finder</h2>
                    <ul>
                        <li>Ability for Customers / Advertiser to setup an account themselves</li>
                        <li>Ability for Customer / Advertiser to post a job advertisement themselves</li>
                        <li>Ability for Job Seeker to setup an account themselves</li>
                        <li>Ability for Job Seeker to post a profile themselves</li>
                        <li>Ability for job advertisement and job seeker matching based on numerous criteria including skills, experience, location, etc</li>
                        <li>Ability for automated job alerts to be setup/configured</li>
                        <li>Automated processes for managing account timeliness and dormancy</li>
                        <li>Automated processes for managing job advertisement timeliness and dormancy
                        </li>
                    </ul>
                </div>

            </div>
        </section>
    </div>
</div>






</main>

<?php
include "../../footer.php"
?>

    <?php
    include "../../assets_footer.php"
    ?>
</html>
